function [p, ke_corr, perm_corrs, z, bias_corrected_corr] = pval_perm_Kendall(A, B, do_perm, varargin)
    % [p, dist, perm_dists, z, bias_corrected_dist] = pval_perm_Kendall(A, B, do_perm, n_rept, isRSM)
    %
    % Calculates the Kendall's tau correlation of two representational matrices
    % inputs:
    ...
    % outputs:
    %   - ke_corr: Kendall's tau correlation of A and B
    ...
    ke_corr = corr(vectorize(A), vectorize(B), 'type', 'Kendall');    
    ...
    if do_perm
        ...
        for perm_i = 1 : n_rept
            rnd_perm = randperm(L);
            perm_corrs(perm_i) = corr(vectorize(A), vectorize(B(rnd_perm, rnd_perm)), 'type', 'Kendall');
        end
        ...
        z = (ke_corr - mu)/sigma;
        bias_corrected_corr = ke_corr - mu;
    else
        p = 0;
        z = 0;
