...
for s=1:nSub
    ...
    [rc(s,1),~, rc_riem_G(s,1), rc_cka(s,1)] = data2rc(X{s},Y{s},'Correlation');
end
...
if opt.nRandomisation == 1 %if only one, then don't bother with parfor like below
    ...
    for s=1:nSub % Ensure reasonably accurate estimate
        ...
        [brc(s,iter),~, brc_riem_G(s,iter),brc_cka(s,iter)] = data2rc(bX,bY,'Correlation');
    end
elseif opt.nRandomisation > 1
    for s=1:nSub % Ensure reasonably accurate estimate
        ...
        parfor iter = 1:opt.nRandomisation
            ...
            [tmp_brc{iter},~, tmp_brc_riem_G{iter},tmp_brc_cka{iter}] = data2rc(bX,bY,'Correlation');
        end
