function [rc_w,rc_riem_rsm, rc_riem_G, rc_cka] = data2rc(X,Y,metric,corrType)
...
%% ====  Riemannian ====
% if (nTimes/min(nVoxs))<=0.8

for r=1:nruns
    Gs1(:, :, r) = secondMoment(X{r}, 0);
    Gs2(:, :, r) = secondMoment(Y{r}, 0);
    
    rdms1(:, :, r)    = 1 - squareform(pdist(X{r},metric));
    rdms2(:, :, r)    = 1 - squareform(pdist(Y{r},metric));
end
    ...
    for i = 1:nruns
        ...
        if (min(eig(Gs2(:, :, r))) <=eps )
            ...
            warning('Gs2 matrix shrinked!')
        
        end
        
        if (min(eig(rdms1(:, :, r))) <=eps )
            rdms1(:, :, r) = Riemann_shrink(rdms1(:, :, r));
            warning('rdm2 matrix shrinked!')
        
        end
        
        if (min(eig(rdms2(:, :, r))) <=eps )
            rdms2(:, :, r) = Riemann_shrink(rdms2(:, :, r));
            warning('rdm2 matrix shrinked!')
        
        end
    end
    
dist_riem_rsm = zeros(nruns,1);

dist_riem_G  = zeros(nruns,1);
for i = 1:nruns
    dist_riem_G(i) = distance_riemann(Gs1(:, :, i), Gs2(:, :, i));
    dist_riem_rsm(i) = distance_riemann(rdms1(:, :, i), rdms2(:, :, i));
end
rc_riem_G = mean(dist_riem_G);
rc_riem_rsm = mean(dist_riem_G);
